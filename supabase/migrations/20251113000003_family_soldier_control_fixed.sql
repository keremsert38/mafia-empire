/*
  # Aile Soldato Kontrol Sistemi - Düzeltilmiş Versiyon
  
  Bu migration aşağıdaki özellikleri ekler:
  1. Aile üyelerine soldato kontrolü yetkisi
  2. Capo ve Sottocapo'nun soldatolara komut verebilmesi
  3. Yeni aile kurulduğunda otomatik malikane atanması
  4. Rakip ailelerin malikânelerine saldırı sistemi
*/
...
DROP TABLE IF EXISTS family_soldier_assignments CASCADE;
CREATE TABLE family_soldier_assignments (
...
DROP POLICY IF EXISTS "Family members can view assignments" ON family_soldier_assignments;
...
DROP POLICY IF EXISTS "Leaders can manage soldier assignments" ON family_soldier_assignments;
...
-- Yeni aile kurulduğunda otomatik malikane atama fonksiyonu (şimdilik devre dışı)
...
-- Trigger oluştur (şimdilik devre dışı)
...
-- Log
SELECT 'Family soldier control system created successfully - FIXED VERSION' as message;
