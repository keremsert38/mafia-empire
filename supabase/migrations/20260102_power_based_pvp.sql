/*
  # Güç Bazlı PvP Savaş Sistemi
  
  YENİ KURALLAR:
  1. Güç = (Silahsız Soldato × 5) + (Silahlı Soldato × 6)
  2. Kimin gücü fazlaysa O kazanır
  3. Kazanan ödül alır: Güç × 100 cash
  4. Push notification eklendi
*/

-- Güncellenmiş rpc_attack_player - GÜÇ BAZLI + PUSH NOTIFICATION
...
BEGIN
  ...
  -- Saldıran bilgileri
  SELECT username, soldiers, COALESCE(weapon, 0), cash, battles_won, battles_lost
  INTO v_attacker_name, v_attacker_soldiers, v_attacker_weapons, v_attacker_cash, v_attacker_battles_won, v_attacker_battles_lost
  FROM player_stats
  WHERE id = v_attacker_id;
  
  -- Savunucu bilgileri
  SELECT username, soldiers, COALESCE(weapon, 0), cash, battles_lost, expo_push_token
  INTO v_defender_name, v_defender_soldiers, v_defender_weapons, v_defender_cash, v_defender_battles_lost, v_defender_token
  FROM player_stats
