-- =====================================================
-- ÇANTA VE MARKET ENTEGRASYONU
-- player_stats'taki ürünleri okur ve markete satışa koymayı sağlar
-- =====================================================

-- =====================================================
-- 1. ENVANTER GÖRÜNTÜLEME (player_stats'tan oku)
-- =====================================================
...
BEGIN
    ...
    SELECT 
        ...
        COALESCE(r.base_cost, r.base_value, 100)::INTEGER
    FROM resources r WHERE LOWER(r.name) LIKE '%demir cevheri%' AND v_stats.demir_cevheri > 0
    
    UNION ALL
    SELECT r.id, r.name::TEXT, COALESCE(r.image_url, 'https://images.unsplash.com/photo-1524661135-423995f22d0b?w=200')::TEXT, v_stats.kereste, COALESCE(r.base_cost, r.base_value, 100)::INTEGER
    FROM resources r WHERE LOWER(r.name) LIKE '%kereste%' AND v_stats.kereste > 0
    
    UNION ALL
    SELECT r.id, r.name::TEXT, COALESCE(r.image_url, 'https://images.unsplash.com/photo-1524661135-423995f22d0b?w=200')::TEXT, v_stats.barut, COALESCE(r.base_cost, r.base_value, 100)::INTEGER
    FROM resources r WHERE LOWER(r.name) LIKE '%barut%' AND v_stats.barut > 0
    
    UNION ALL
    SELECT r.id, r.name::TEXT, COALESCE(r.image_url, 'https://images.unsplash.com/photo-1524661135-423995f22d0b?w=200')::TEXT, v_stats.demir_kulce, COALESCE(r.base_cost, r.base_value, 100)::INTEGER
    FROM resources r WHERE LOWER(r.name) LIKE '%demir külçe%' AND v_stats.demir_kulce > 0
    
    UNION ALL
    SELECT r.id, r.name::TEXT, COALESCE(r.image_url, 'https://images.unsplash.com/photo-1524661135-423995f22d0b?w=200')::TEXT, v_stats.namlu, COALESCE(r.base_cost, r.base_value, 100)::INTEGER
    FROM resources r WHERE LOWER(r.name) LIKE '%namlu%' AND v_stats.namlu > 0
    
    UNION ALL
    SELECT r.id, r.name::TEXT, COALESCE(r.image_url, 'https://images.unsplash.com/photo-1524661135-423995f22d0b?w=200')::TEXT, v_stats.kabze, COALESCE(r.base_cost, r.base_value, 100)::INTEGER
    FROM resources r WHERE LOWER(r.name) LIKE '%kabze%' AND v_stats.kabze > 0
    
    UNION ALL
    SELECT r.id, r.name::TEXT, COALESCE(r.image_url, 'https://images.unsplash.com/photo-1524661135-423995f22d0b?w=200')::TEXT, v_stats.tohum, COALESCE(r.base_cost, r.base_value, 100)::INTEGER
    FROM resources r WHERE LOWER(r.name) LIKE '%tohum%' AND v_stats.tohum > 0
    
    UNION ALL
    SELECT r.id, r.name::TEXT, COALESCE(r.image_url, 'https://images.unsplash.com/photo-1524661135-423995f22d0b?w=200')::TEXT, v_stats.ilac, COALESCE(r.base_cost, r.base_value, 100)::INTEGER
    FROM resources r WHERE (LOWER(r.name) LIKE '%ilaç%' OR LOWER(r.name) LIKE '%ilac%') AND v_stats.ilac > 0
    
    UNION ALL
    SELECT r.id, r.name::TEXT, COALESCE(r.image_url, 'https://images.unsplash.com/photo-1524661135-423995f22d0b?w=200')::TEXT, v_stats.saglik_kiti, COALESCE(r.base_cost, r.base_value, 100)::INTEGER
    FROM resources r WHERE (LOWER(r.name) LIKE '%sağlık%' OR LOWER(r.name) LIKE '%saglik%') AND v_stats.saglik_kiti > 0
    
    UNION ALL
    SELECT r.id, r.name::TEXT, COALESCE(r.image_url, 'https://images.unsplash.com/photo-1524661135-423995f22d0b?w=200')::TEXT, v_stats.yemek, COALESCE(r.base_cost, r.base_value, 100)::INTEGER
    FROM resources r WHERE LOWER(r.name) LIKE '%yemek%' AND v_stats.yemek > 0
    
    UNION ALL
    SELECT r.id, r.name::TEXT, COALESCE(r.image_url, 'https://images.unsplash.com/photo-1524661135-423995f22d0b?w=200')::TEXT, v_stats.baretta, COALESCE(r.base_cost, r.base_value, 100)::INTEGER
    FROM resources r WHERE LOWER(r.name) LIKE '%baretta%' AND v_stats.baretta > 0
    
    UNION ALL
    SELECT r.id, r.name::TEXT, COALESCE(r.image_url, 'https://images.unsplash.com/photo-1524661135-423995f22d0b?w=200')::TEXT, v_stats.ak47, COALESCE(r.base_cost, r.base_value, 100)::INTEGER
...
-- =====================================================
-- 2. MARKET İLANI OLUŞTURMA (player_stats'tan düş)
-- =====================================================
...
BEGIN
    ...
    -- Fiyat kontrolü
    v_max_price := COALESCE(v_resource.base_cost, v_resource.base_value, 100) * 2;
    ...
    INSERT INTO marketplace_listings (seller_user_id, resource_id, quantity, price_per_unit, is_active)
...
-- =====================================================
-- 3. MARKET'TEN SATIN ALMA (player_stats'a ekle)
-- =====================================================
...
BEGIN
    ...
    -- Satıcıya para ekle
    UPDATE player_stats SET cash = cash + v_total_cost WHERE id = v_listing.seller_user_id;
